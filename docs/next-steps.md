...
- **Pagina Feedback** (2026-07-24): nuova bacheca `/feedback` dove chiunque puo scrivere feedback/segnalazioni e vedere l'elenco completo (con stato sempre visibile); solo l'autore modifica il proprio testo; solo un admin segna come fatto, assegna una priorita (alta/media/bassa) o elimina (singolo, selezione multipla o tutti insieme, sempre con conferma). Nuova tabella `feedback` (`supabase/migrations/20260724100000_feedback_board.sql`) con una scelta architetturale degna di nota: RLS e row-level, non column-level, quindi una normale policy "l'autore aggiorna la propria riga" avrebbe permesso anche di modificarne stato/priorita; niente policy `update` diretta sulla tabella, ogni mutazione passa da una delle due RPC `security definer` (`update_feedback_content`, solo autore; `update_feedback_status`, solo admin), ciascuna con il proprio controllo sul campo che puo toccare. Nessun Supabase Realtime (a differenza della votazione, il feedback e asincrono, non una sessione collaborativa dal vivo). Verificato: query REST dirette per confermare tabella/RPC applicate correttamente, e una pagina di debug temporanea (con dati finti, poi rimossa) per verificare visivamente vista admin/membro, filtri, modifica inline e modal di conferma eliminazione.
